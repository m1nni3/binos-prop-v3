...
export function renderContacts(container) {
  ...
  function rerender() {
    ...
    let html = `
      <div class="flex flex-col h-full">
          ...
          <div class="px-5 py-4 border-b border-binos-border bg-white">
            <div class="flex flex-wrap items-center justify-between gap-4">
              <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="relative">
                  <select class="select-field py-2 pr-8 text-sm" data-filter-role>
                    <option value="all">All Roles</option>
                    ${roles.map(r => `<option value="${escHtml(r)}" ${state.filterRole === r ? 'selected' : ''}>${escHtml(r)}</option>`).join('')}
                  </select>
                </div>
                <div class="relative">
                  <select class="select-field py-2 pr-8 text-sm" data-filter-company>
              ...
              <div class="flex items-center gap-3">
                <button class="btn-primary btn-sm" data-show-form>
                  <span class="mr-1">${ICONS.plus}</span> Add Contact
                </button>
                <button class="btn-secondary btn-sm" data-export-csv>
            ...
            <div class="px-5 py-4 border-t border-binos-border bg-white">
              <div class="flex items-center justify-between">
                ...
                <div class="flex gap-2">
                  <button class="btn-secondary btn-sm" data-page-prev ${!paginated.hasPrev ? 'disabled' : ''}>
                    Previous
                  </button>
                  <button class="btn-secondary btn-sm" data-page-next ${!paginated.hasNext ? 'disabled' : ''}>
                    Next
                  </button>
                </div>
              </div>
        ...
        ${state.showForm ? `
          <div class="modal-overlay" data-close-form>
            <div class="modal-content max-w-2xl" data-stop-propagation>
              <div class="card-header">
                <h3 class="font-display font-semibold">${state.editingId ? 'Edit Contact' : 'Add New Contact'}</h3>
                <button class="p-1.5 rounded-lg hover:bg-binos-light" data-close-form>
              ...
              <div class="card-body">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div class="md:col-span-2">
                    ...
                    <input type="text" class="input-field" data-form-name value="${escHtml(state.form.name)}" required>
                  </div>
                  <div>
                    <label class="block text-sm font-medium text-binos-gray mb-1.5">Role / Job Title</label>
                    <input type="text" class="input-field" data-form-role value="${escHtml(state.form.role || '')}">
                  </div>
                  <div>
                    <label class="block text-sm font-medium text-binos-gray mb-1.5">Category</label>
                    <select class="select-field" data-form-category>
                  ...
                  <div>
                    ...
                    <input type="text" class="input-field" data-form-company value="${escHtml(state.form.company || '')}">
                  </div>
                  <div>
                    <label class="block text-sm font-medium text-binos-gray mb-1.5">Office</label>
                    <input type="text" class="input-field" placeholder="e.g. Durban, Cape Town" data-form-office value="${escHtml(state.form.office || '')}">
                  </div>
                  <div>
                    <label class="block text-sm font-medium text-binos-gray mb-1.5">Telephone</label>
                    <input type="tel" class="input-field" data-form-phone value="${escHtml(state.form.phone || '')}">
                  </div>
                  <div>
                    <label class="block text-sm font-medium text-binos-gray mb-1.5">Office Number</label>
                    <input type="tel" class="input-field" data-form-office_number value="${escHtml(state.form.office_number || '')}">
                  </div>
                  <div>
                    <label class="block text-sm font-medium text-binos-gray mb-1.5">Email</label>
                    <input type="email" class="input-field" data-form-email value="${escHtml(state.form.email || '')}">
                  </div>
                  <div>
                  ...
                  <div class="md:col-span-2">
                    ...
                    <input type="text" class="input-field" data-form-address value="${escHtml(state.form.address || '')}">
                  </div>
                  <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-binos-gray mb-1.5">Website</label>
                    <input type="url" class="input-field" data-form-website value="${escHtml(state.form.website || '')}">
                  </div>
                  <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-binos-gray mb-1.5">Notes</label>
                    <textarea class="input-field min-h-[80px]" data-form-notes>${escHtml(state.form.notes || '')}</textarea>
                  </div>
                  ${state.form.office ? `
                    <div class="md:col-span-2 pt-2 border-t border-binos-border/50">
                      <label class="flex items-start gap-3 cursor-pointer">
                        <input type="checkbox" class="mt-0.5 rounded border-binos-border text-binos-blue focus:ring-binos-blue/30" data-form-applyOffice ${state.form.applyOffice ? 'checked' : ''}>
                        <div>
                ...
                <div class="flex gap-3 pt-6">
                  <button class="btn-secondary flex-1" data-close-form>Cancel</button>
                  <button class="btn-primary flex-1" data-save-contact ${!state.form.name ? 'disabled' : ''}>Save Contact</button>
                </div>
              </div>
            </div>
          </div>
        ...
        ${state.showDeleteConfirm ? `
          <div class="modal-overlay" data-dismiss-delete>
            <div class="modal-content max-w-md" data-stop-propagation>
              <div class="p-6">
                ...
                <div class="flex gap-3">
                  <button class="btn-secondary flex-1" data-dismiss-delete>Cancel</button>
                  <button class="btn-danger flex-1" data-confirm-delete data-confirm-id="${state.showDeleteConfirm}">Delete</button>
                </div>
              </div>
            </div>
          </div>
    ...
    return `
      <div class="card p-4 hover:shadow-card-hover transition-shadow border-l-4 border-${colorClass}-500">
        ...
        <div class="space-y-1.5 text-sm">
          ...
          ${contact.address ? `
            <div class="flex items-start gap-2 text-binos-gray">
              <svg class="w-3.5 h-3.5 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0 conversationalAI/></svg>
        ...
        <div class="mt-3 flex gap-2">
          <button class="btn-secondary btn-sm py-1.5 px-3" data-edit-contact="${contact.id}">
            Edit
          </button>
          <button class="btn-danger btn-sm py-1.5 px-3" data-delete-contact="${contact.id}">
            Delete
          </button>
        </div>
      </div>
  ...
  function attachEvents() {
    ...
    container.querySelectorAll('[data-filter-role]').forEach(el => {
      el.onchange = () => { state.filterRole = el.value; state.page = 1; rerender(); };
    });
    container.querySelectorAll('[data-filter-company]').forEach(el => {
      el.onchange = () => { state.filterCompany = el.value; state.page = 1; rerender(); };
    });
    ...
    container.querySelectorAll('[data-page-next]').forEach(el => {
      el.onclick = () => { if (state.page * getPaginatedContacts().total > state.page * state.pageSize) { state.page++; rerender(); } };
    });
    container.querySelectorAll('[data-edit-contact]').forEach(el => {
      el.onclick = () => editContact(parseInt(el.dataset.editContact));
    });
    container.querySelectorAll('[data-delete-contact]').forEach(el => {
      el.onclick = () => deleteContact(parseInt(el.dataset.deleteContact));
    });
    ...
    container.querySelectorAll('[data-dismiss-delete]').forEach(el => {
      el.onclick = () => { state.showDeleteConfirm = null; rerender(); };
    });
    container.querySelectorAll('[data-confirm-delete]').forEach(el => {
      el.onclick = () => confirmDelete(parseInt(el.dataset.confirmId));
    });
    container.querySelectorAll('[data-form-name]').forEach(el => {
      el.onchange = () => { state.form.name = el.value; rerender(); };
    });
    container.querySelectorAll('[data-form-role]').forEach(el => {
      el.onchange = () => { state.form.role = el.value; rerender(); };
    });
    container.querySelectorAll('[data-form-company]').forEach(el => {
      el.onchange = () => { state.form.company = el.value; rerender(); };
    });
    container.querySelectorAll('[data-form-office]').forEach(el => {
      el.onchange = () => { state.form.office = el.value; rerender(); };
    });
    container.querySelectorAll('[data-form-phone]').forEach(el => {
      el.onchange = () => { state.form.phone = el.value; rerender(); };
    });
    container.querySelectorAll('[data-form-office_number]').forEach(el => {
      el.onchange = () => { state.form.office_number = el.value; rerender(); };
    });
    container.querySelectorAll('[data-form-email]').forEach(el => {
      el.onchange = () => { state.form.email = el.value; rerender(); };
    });
    container.querySelectorAll('[data-form-category]').forEach(el => {
      el.onchange = () => { state.form.category = el.value; rerender(); };
    });
    container.querySelectorAll('[data-form-property_id]').forEach(el => {
      el.onchange = () => { state.form.property_id = el.value || null; rerender(); };
    });
    container.querySelectorAll('[data-form-address]').forEach(el => {
      el.onchange = () => { state.form.address = el.value; rerender(); };
    });
    container.querySelectorAll('[data-form-website]').forEach(el => {
      el.onchange = () => { state.form.website = el.value; rerender(); };
    });
    container.querySelectorAll('[data-form-notes]').forEach(el => {
      el.onchange = () => { state.form.notes = el.value; rerender(); };
    });
    container.querySelectorAll('[data-form-applyOffice]').forEach(el => {
      el.onchange = () => { state.form.applyOffice = el.checked; rerender(); };
    });
